...
function start_server {
    ...
    if [ "$server_type" = "jar" ]; then
      ...
      java $javaagent_arg -Xms1024m -Xmx1024m -jar $server_package 2> $server_output  &
    else
...
for agent_jar in $agent_jars; do
    ...
    server_start_cpu=$(ps -p $server_pid -o etimes | awk 'FNR == 2 {print}' | xargs)
    ...
    server_stop_cpu=$(ps -p $server_pid -o etimes | awk 'FNR == 2 {print}' | xargs)
    ...
    server_max_rss=$(grep VmHWM /proc/$server_pid/status | awk '{print $2}')
